...
const getWebtoonInfo = async (page) => {
  const webtoonInfo = await page.evaluate(() => {
    ...
    const view = document.querySelector("#content > div:nth-child(2)");
    const interest = view.querySelector(
      "div > button > span:nth-child(3)"
    ).innerText;
    const firstEpLink = view.querySelector("div > a").href;
    const firstEpTitle = view.querySelector(
...
const getWebtoonData = async (page, data) => {
  ...
  const webtoonList = await page.evaluate(() => {
    ...
    for (let webtoon of webtoons) {
      ...
      const rating = webtoon.querySelector("div > div > span > span").innerText;
      const step = [100, 99, 98, 96, 94, 92, 90, 70, 40, 0].find(
        (n) => n <= Number(rating) * 10
      );

      list.push({ id, rating, viewCount, step, state });
    }
